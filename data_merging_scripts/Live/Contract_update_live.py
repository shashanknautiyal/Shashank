...
# take master-table file 
complete_dump = spark.read.option("header", "true").option("delimiter", "|").option("inferSchema","true").csv("s3://cmh-process-data/master-tables/master_table_Contract_data/Contract_data.csv")
complete_dump.createOrReplaceTempView("complete_dump")

# change path of current file dynamic
incremental_dump = spark.read.option("header", "true").option("delimiter", "|").option("inferSchema","true").csv("s3://cmh-raw-data/incremental-load/contract-data/contract-data_"+dt+".csv")
