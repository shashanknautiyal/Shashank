...
complete_dump = spark.read.option("header", "true").option("delimiter", "|").option("inferSchema","true").csv("s3://cmh-raw-data/complete-dump/zsd007n/zsd007n_Till 23072019.csv")
...
incremental_dump = spark.read.option("header", "true").option("delimiter", "|").option("inferSchema","true").csv("s3://cmh-raw-data/incremental-load/zsd007n/*")
...
final_data =spark.sql("select s.CONTRACT_NUMBER, s.INSTALLMENT_DATE, s.PDI_STATUS,s.PDI_TYPE, s.INSTALLMENT_NUMBER,s.DOWNPAYMENT_DATE,s.REGISTRATION_STATUS,s.CUSTOMER_ACCOUNT_NO,s.VAULTED_DATE,s.PDI_RECEIVED_DATE,s.MEMBERSHIP_ID,s.dw_last_modified_date from (select *, row_number() over(partition by unique_key order by dw_last_modified_date desc) as rownum1 from data) s where rownum1 =1 ")
