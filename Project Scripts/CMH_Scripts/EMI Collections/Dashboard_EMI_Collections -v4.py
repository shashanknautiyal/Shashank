...
base_complete = base_complete.withColumn('date', to_date('month_year','yyyy-MM')).withColumn('business_date',lit(business_day))

base_complete=base_complete.withColumn("ODBucketMax",regexp_replace("ODBucketMax", "_", "-"))

# Filtering the data for Current month
base_current_month=base_complete.filter('date>= date_add(last_day(business_date - interval 01 month),1) and date <= last_day(business_date)')
...
# Formating the date Column
inflow_complete = inflow_complete.withColumn('date', to_date('month_year','yyyy-MM')).withColumn('business_date',lit(business_day))

inflow_complete=inflow_complete.withColumn("ODBucketMax",regexp_replace("ODBucketMax", "_", "-"))

# Filtering the data for Current month
inflow_current_month=inflow_complete.filter('date>= date_add(last_day(business_date - interval 01 month),1) and date <= last_day(business_date)')
...
# Formating the date column
target_Complete=target_Complete.withColumn("date",to_date("Target_Month","MMM-yy")).withColumn('business_date',lit(business_day))

# Applying the Current Month Filter
target_current_month=target_Complete.filter('date>= date_add(last_day(business_date - interval 01 month),1) and date <= last_day(business_date)')
#target_current_month=target_Complete.filter('date>= date_add(last_day("2019-12-31" - interval 01 month),1) and date <= last_day("2019-12-31")')
...
base_inflow_2=base_inflow_2.withColumn('business_date',lit(business_day))
base_inflow_2.createOrReplaceTempView('bi2')

## storing the current day of month.
day=spark.sql('select dayofmonth(business_date) as day from base').collect()
day=day[0].day

## Creating Recovery_Target Column as per the business logic
base_inflow_3=spark.sql('select New_Branch,opening_base,inflow as MTD_inflow,Closing_base_target,assumed_inflow,(opening_base - Closing_base_target) + case when  dayofmonth(business_date) <= 7  then greatest(assumed_inflow,inflow) else inflow end as Recovery_Target from bi2')
...
zod_inflow.createOrReplaceTempView('zoi')
...
Holidays = spark.read.option("delimiter","|").csv("s3://cmh-process-data/master-tables/master_table_Holidays_list/Holiday_List.csv",header =True,inferSchema=True)

Holidays = Holidays.withColumn('Date',to_date('Date','dd/MM/yyyy'))

Holidays = Holidays.withColumn("holidays_day_name",date_format(Holidays.Date,'E'))



holidays_current_month_filter = Holidays.filter('Date>= date_add(last_day(now() - interval 01 month),1) and Date <= last_day(now())')

#holidays_current_month_filter = Holidays.filter('Date>= date_add(last_day("2019-12-14" - interval 01 month),1) and Date <= last_day("2019-12-14")')

# Sunday exclude
holidays_current_month = holidays_current_month_filter.filter('holidays_day_name !="Sun"').count()

def sunday_counter():
	counter = 0
	for days in range(1,int(datetime.datetime.today().day+1)):
		day = datetime.datetime.today() - datetime.timedelta(days= days)
...
sunday_counts=sunday_counter()

no_of_working_days = int(datetime.datetime.today().day)- (sunday_counts+holidays_current_month)
...
base_inflow_5=base_inflow_5.withColumn('day_value',lit(no_of_working_days))
...
#emi_dialler.coalesce(1).write.csv("s3://cmh-process-data/test-1/emi_dailler",mode="append",header="true")
...
## New Logic Filter the data take Agent_type = "Tele caller"

emi_master_filtered=emi_master.filter('Agent_type=="Tele Caller"')

emi_master_filtered.createOrReplaceTempView('em')

# Summarising the table on ControlLocation/Branch Level.
emi_master_summ=spark.sql('select New_Branch , count(MemberName) as total_mtd_contacts, count(distinct MemberName) as total_unique_mtd_contacts from em group by New_Branch')
...
field_app=field_app.filter('Rundate = current_date()')
...
con_pay_1=spark.sql(" select sales_document,count(*) as pdi_count from cp where pdi_status= 4 group by sales_document")
con_pay_1.createOrReplaceTempView('cp1')
fut_pdi_df = spark.sql("Select zobi.*, cp1.pdi_count from zobi left join cp1 on zobi.Contract_No=cp1.sales_document")
...
ro_contract_pay_1=spark.sql('Select distinct sales_document as ContractNo from c_p where pdi_returned_date >= date_add(last_day(now() - interval 04 months),1) and pdi_returned_date <=last_day(now() - interval 01 months)')
...
## spark.sql("select sales_document,pdi_returned_date from  cp where sales_document = 413764 order by pdi_returned_date desc ")
...
pdi_modified=spark.sql('Select distinct sales_document as ContractNo from c_p where pdi_received_date between date_add(last_day(now() - interval 01 month),1) and last_day(now()) and installment_date>= date_add(last_day(now()),1)')
...
table_2 = spark.sql('select "Inflow_Table" as Table_name, max(dw_lastupdated) as last_modified, "2020-01-01" as run_date from zoi')
