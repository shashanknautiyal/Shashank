...
#cancel_req= spark.read.option("delimiter",",").csv(r"D:\Shashank\Club Mahindra\S3 data\inflowTree\cancellation_request_table_updated.csv",header=True,inferSchema=True)
...
# Output-1
inflow_can.repartition(1).write.csv("s3://cmh-process-data/test-1/EMI_Inflow",mode="append",header="true")
...
normalised_but_od.createOrReplaceTempView('nbo')
...
active_with_emi_1_5_rz=active_with_emi_1_5_rz.withColumn('flag_normalised',when(active_with_emi_1_5_rz.flag_normalised=='Normalised_but_back_in_OD','Normalised_but_back_in_OD').otherwise('First_time_inflow'))
...
active_with_morethan_5_emi_rz=spark.sql("select awm5.*,rfl9.flag_normalised from awm5 left join rfl9 on awm5.Contract_No=rfl9.Contract_No")
...
#zfi=spark.read.option("delimiter","|").csv("D:\Shashank\Club Mahindra\S3 data\Zfi065\ZFI065N_OUTPUT_28.02.2019_28.02.2019_08_30_04.csv",header =True,inferSchema=True)
...
## #pdi=spark.read.option("delimiter","|").csv("D:\Shashank\Club Mahindra\S3 data\ZFI pdi\ZFI PDI_12042019.csv",header =True,inferSchema=True)
...
# Check dates filter
pdi_1 = pdi.filter('installment_date>="04-1-2012" or waiver_date>= "04-1-2012" or pdi_realization_date >="04-1-2012"')
...
## change
total_upcoming_pdi= total_pdi.filter('installment_date> "04-1-2012" and installment_date<now()').sort('installment_number')
...
active_with_emi0_rz.repartition(1).write.csv("s3://cmh-process-data/test-1/EMI_Inflow",mode="append",header="true")
...
active_with_emi_1_5_rz.repartition(1).write.csv("s3://cmh-process-data/test-1/EMI_Inflow",mode="append",header="true")
...
active_with_morethan_5_emi_rz.repartition(1).write.csv("s3://cmh-process-data/test-1/EMI_Inflow",mode="append",header="true")	