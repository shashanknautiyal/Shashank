...
active_with_emi0_rz=spark.sql('select Contract_No,case when SaleDate >= date_add(last_day(now() - interval 03 months),1) and  SaleDate <= last_day(now() - Interval 02 months) then "FIF" else "upgrade/ppc" end as Flag from awe')
...
# setting an offset date to filter the last 09 months Contracts in recovery fileLast 09 Months
recovery_file=spark.sql('select *, date_add(last_day(now() - interval 11 months),1) as offset from rf')
...
pdi_none_contracts=spark.read.option("delimiter",",").csv("s3://cmh-process-data/master-tables/master_table_pdinone_prevmonth/pdinone_contracts.csv",header=True,inferSchema=True)
