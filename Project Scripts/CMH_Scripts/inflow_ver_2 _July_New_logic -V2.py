...
# Output-1
inflow_can.coalesce(1).write.csv("s3://cmh-process-data/intermediate_files/EMI_Inflow/inflow_can",mode="append",header="true")

cmd="aws s3 rm s3://cmh-process-data/intermediate_files/EMI_Inflow/inflow_can/Inflow_cancel.csv"
os.system(cmd)
...
file_path = "s3://cmh-process-data/intermediate_files/EMI_Inflow/inflow_can/"
created_file_path = fs.globStatus(Path(file_path + "part*.csv"))[0].getPath()
fs.rename(created_file_path,Path(file_path + "Inflow_cancel.csv"))
...
#zsd=spark.read.option("delimiter","|").csv("s3://cmh-process-data/master-tables/master_table_zsd_007_monthly/ZSD_007N.csv",header=True,inferSchema=True)
...
#zsd_filter= spark.sql('select * from zsd where INSTALLMENT_DATE >= date_add(last_day(now() - interval 02 month),1) and INSTALLMENT_DATE<= last_day(now() - interval 01 month) and PDI_STATUS="01"')
...
#zfi_65_1_feb = zfi_65_1_feb.withColumn("odbucketmax",regexp_replace("odbucketmax", "[^a-zA-Z0-9_-]", " "))

#zfi_65_summ=spark.sql("Select contractno from zfeb where status='Active' and odbucketmax != ' ' and mortgagecheck=1")
...
#zsd_excluded.coalesce(1).write.csv("s3://cmh-process-data/master-tables/master_table_pdi_none",mode="append",header="true")
...
active_with_emi0_rz.coalesce(1).write.csv("s3://cmh-process-data/intermediate_files/EMI_Inflow/Active0",mode="append",header="true")

cmd="aws s3 rm s3://cmh-process-data/intermediate_files/EMI_Inflow/Active0/active_zero.csv"
os.system(cmd)
...
fs = FileSystem.get(URI("s3://cmh-process-data"), sc._jsc.hadoopConfiguration())
file_path = "s3://cmh-process-data/intermediate_files/EMI_Inflow/Active0/"
created_file_path = fs.globStatus(Path(file_path + "part*.csv"))[0].getPath()
fs.rename(created_file_path,Path(file_path + "active_zero.csv"))
...
active_with_emi_1_5_rz.coalesce(1).write.csv("s3://cmh-process-data/intermediate_files/EMI_Inflow/active_1_5",mode="append",header="true")

cmd="aws s3 rm s3://cmh-process-data/intermediate_files/EMI_Inflow/active_1_5/active_1_5.csv"
os.system(cmd)
...
fs = FileSystem.get(URI("s3://cmh-process-data"), sc._jsc.hadoopConfiguration())
file_path = "s3://cmh-process-data/intermediate_files/EMI_Inflow/active_1_5/"
created_file_path = fs.globStatus(Path(file_path + "part*.csv"))[0].getPath()
fs.rename(created_file_path,Path(file_path + "active_1_5.csv"))
...
## output-4
active_with_morethan_5_emi_rz.coalesce(1).write.csv("s3://cmh-process-data/intermediate_files/EMI_Inflow/active_more_5",mode="append",header="true")

cmd="aws s3 rm s3://cmh-process-data/intermediate_files/EMI_Inflow/active_more_5/active_more_5.csv"
os.system(cmd)
...
fs = FileSystem.get(URI("s3://cmh-process-data"), sc._jsc.hadoopConfiguration())
file_path = "s3://cmh-process-data/intermediate_files/EMI_Inflow/active_more_5/"
created_file_path = fs.globStatus(Path(file_path + "part*.csv"))[0].getPath()
fs.rename(created_file_path,Path(file_path + "active_more_5.csv"))	
