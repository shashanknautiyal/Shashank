...
base_summ=spark.sql("Select New_Branch, Count(*) as opening_base from b group by New_Branch order by New_Branch")

base_summ.createOrReplaceTempView('bs')

inflow_summ=spark.sql("Select New_Branch, Count(*) as inflow from if group by New_Branch order by New_Branch")
...
emi_dialler=spark.sql("Select drl.Campaign_name, drl.Start_time, drl.Length_in_sec,dd.AgentName,dd.MemberName,dd.ContractId,dd.CallStatus from drl left join dd on drl.Lead_id=dd.ID")
...
con_pay=con_pay.select('reference_document','pdi_type','pdi_status','pdi_realization_date','installment_date','pdi_received_date','pdi_returned_date','pdi_realization_date')
con_pay.createOrReplaceTempView('cp')
...
contract_pay=con_pay.select('reference_document','pdi_status','installment_date','pdi_received_date','pdi_returned_date','pdi_realization_date')
...
ro_contract_pay_1=spark.sql('Select reference_document as ContractNo,pdi_status,installment_date,pdi_received_date,pdi_returned_date,pdi_realization_date from rcp where pdi_returned_date between date_add(last_day("2019-02-14" - interval 04 months),1) and last_day("2019-02-14" - interval 01 months)')
