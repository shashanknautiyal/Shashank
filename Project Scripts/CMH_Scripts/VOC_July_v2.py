...
## take Zone from inflow and base not from contract data
#### Zfi_65  ####

# S3 path

zfi_65_1_feb =spark.read.option("delimiter","|").csv(r"ss3://cmh-process-data/master-tables/master_table_ZFI_065_daily/ZFI_065_Daily.csv",header=True,inferSchema=True)


zfi_65_1_feb=zfi_65_1_feb.withColumnRenamed('ContractNo','Contract_No').withColumnRenamed('MortgageID','Mortgage_ID').withColumnRenamed('OD Bucket Max','OD_Bucket_Max').withColumnRenamed('Mortgage Check','Mortgage_Check').withColumnRenamed('EMI Realized count','EMI_Realized_count').withColumnRenamed('Sale date','Sale_date').withColumnRenamed('Member ID','Member_ID').withColumnRenamed('Realized Percent Con','Realized_percent_con')
...
zfi_filter=zfi_filter.withColumn('Sale_vintage',(datediff(lit(date_add(last_day(current_date() - interval 01 months),1)),zfi_filter.Sale_date))/30.5)
...
reservation_sel=reservation_sel.withColumn('ReservationStatusID',reservation_sel.ReservationStatusID)

## Remark - Cast ReservationStatusID to Integer


# change Dynamic check between taking outer values
reservation_filter=reservation_sel.filter('dBookingDate < date_add(last_day(current_date() - interval 01 months),1) and CheckIn < date_add(last_day(current_date() - interval 01 months),1) and ReservationStatusID between 21 and 24')
...
# Change Dynamic
zfi_reservation_final=zfi_reservation_final.withColumn('Time_since_last_holiday',round((datediff(lit date_add(last_day(current_date() - interval 01 month),1)),zfi_reservation_final.Last_HD))/30.5,2))
...
### take zone from base and inflow file only
...
#### add filter on dialer data with last 3 months
...
zfi_reservation_final_7=zfi_reservation_final_7.withColumn('run_date',lit(current_date()))
