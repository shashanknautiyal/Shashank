...
pace_full=spark.read.option("delimiter","|").csv("s3://cmh-raw-data/complete-dump/pace-dialer/pace-dialer_from Mar 19 to Jul 19.csv",header =True,inferSchema=True)

pace_full=pace_full.withColumn('StartTime',to_date('StartTime',"yyyy-MM-dd").cast('date'))

pace=pace_full.filter('Date >= date_add(last_day(now() - interval 01 months),1) and Date<=(last_day(now()))')
...
pace_daywise.repartition(1).write.csv("s3://cmh-process-data/test-1/Pace_dialler/Pace_daywise",mode="append",header="true")
...
pace_effort_metrics.repartition(1).write.csv("s3://cmh-process-data/test-1/Pace_dialler/Pace_effort_metrics",mode="append",header="true")
...
pace_daily_report.repartition(1).write.csv("s3://cmh-process-data/test-1/Pace_dialler/Pace_daily_report",mode="append",header="true")
