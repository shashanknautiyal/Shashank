...
zfi_65_28_feb = zfi_65_28_feb.withColumn("ODBucketMax",regexp_replace(col("ODBucketMax"), "[^a-zA-Z0-9_-]", " "))
...
con_pay_select=con_pay_select.withColumn('pdi_returned_date',to_date('pdi_returned_date',"MM/dd/yyyy")).withColumn('pdi_received_date', to_date('pdi_received_date', "MM/dd/yyyy")).withColumn('installment_date', to_date('installment_date', "MM/dd/yyyy")).withColumn('pdi_status',con_pay_select.pdi_status.cast('Integer'))
...
foreclosed_data =  spark.sql("select Contract_No, min(Realized_per_con) as Realized_per_con from fd groupBy Contract_No ")
...
recovery_july_filter= recovery_feb.filter('rec_date >= date_add(last_day(now() - interval 1 months),1) and rec_date <= last_day(now())')
