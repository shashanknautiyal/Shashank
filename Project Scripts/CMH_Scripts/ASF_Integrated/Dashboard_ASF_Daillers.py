...
branch_full=branch_full.withColumn('business_date',lit(business_day))
...
branch= branch_full.filter('DateofCalling>= date_add(last_day(business_date - interval 01 month),1) and DateofCalling <= last_day(business_date)')
...
field_visit_full=field_visit_full.withColumn('business_date',lit(business_day))
#for col in field_visit_full.columns:
	#field_visit_full=field_visit_full.withColumnRenamed(col,col.lower())

field_visit_full=field_visit_full.withColumn('InteractionDate',to_date('InteractionDate','yyyy-MM-dd')).withColumn('MemberID',field_visit_full.MemberID.cast('Integer'))

field_visit= field_visit_full.filter('InteractionDate>= date_add(last_day(business_date - interval 01 month),1) and InteractionDate <= last_day(business_date)')
...
emi_dialler=emi_dialler.withColumn('business_date',lit(business_day))

## T & E : 26-12-2019

emi_dialler = emi_dialler.withColumn('last_modified_date',to_date('last_modified_date','yyyy-MM-dd'))

emi_dialler = emi_dialler.filter('last_modified_date >= date_add(last_day( business_date - interval 01 months),1) and last_modified_date <= (last_day(business_date))') 
...
branch_benchmarks=spark.sql('select *, case when control_location in ("bangalore-koramangala","bangalore-millersroad") then "bangalore" when control_location in ("delhi-bikaji","delhi-jasola","delhi-pitampura") then "delhi" when control_location in ("mumbai-andheri","mumbai-vashi") then "mumbai" else control_location end as control_location_1 from bb')

branch_benchmarks=branch_benchmarks.drop('control_location')

branch_benchmarks=branch_benchmarks.withColumnRenamed('control_location_1','control_location')
...
branch_benchmarks_old=branch_benchmarks_old.withColumn('run_date',to_date('run_date','yyyy-MM-dd'))
...
branch_benchmarks_final_2=branch_benchmarks_final_2.withColumn('run_date',branch_benchmarks_final_2.run_date.cast('string'))
...
pace_full=pace_full.withColumn('business_date',lit(business_day))


#pace=pace_full.filter('Date >= date_add(last_day(now() - interval 03 months),1) and Date<=(last_day(now()-interval 02 months))')


pace=pace_full.filter('StartTime >= date_add(last_day(business_date - interval 01 months),1) and StartTime<=(last_day(business_date))')
...
nhm=nhm.withColumn('business_date',lit(business_day))


#nhm_current=nhm.filter('Date >= date_add(last_day(now() - interval 03 months),1) and Date<=(last_day(now() - interval 02 months))')
nhm_current=nhm.filter('Date >= date_add(last_day(business_date - interval 01 months),1) and Date<=(last_day(business_date))')
...
chennai=chennai.withColumn('business_date',lit(business_day))
...
chennai_integrated_2 = spark.sql('select "Chennai ASF" as Channel,round(sum(attempt_mtd)/sum(no_of_working_days),2) as mtd_attempt,round(sum(contact_mtd)/sum(no_of_working_days),2) as mtd_contacted,round(sum(talk_duration_per_exec_secs)/sum(contact_mtd),2) as mtd_average_call_time,round((sum(talk_duration_per_exec_secs)/sum(no_of_working_days))/60,2) mtd_talktime_mins,round(count(case when talk_duration_per_exec_mins < 75 then Agent end ),2) as mtd_executives_less_than_75mins from cdm')
...
pace_nhm_chennai_old=pace_nhm_chennai_old.withColumn('run_date',to_date('run_date','yyyy-MM-dd'))
...
pace_nhm_chennai_final=pace_nhm_chennai_final.withColumn('run_date',pace_nhm_chennai_final.run_date.cast('string'))
...
table_details_old=table_details_old.withColumn('run_date',to_date('run_date','yyyy-MM-dd')).withColumn('last_modified',to_date('last_modified','yyyy-MM-dd'))
